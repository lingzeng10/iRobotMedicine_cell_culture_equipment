...
import { ProductionTarget, UpdateTargetRequest } from '../types/target';
import { TargetService } from '../services/targetApi';

// 編輯預生產目標表單元件屬性介面
interface EditTargetFormProps {
  open: boolean; // 對話框是否開啟
  target: ProductionTarget | null; // 要編輯的目標
  onClose: () => void; // 關閉對話框回調函數
  onSuccess: (targetId: string, updatedTarget: ProductionTarget) => void; // 成功更新回調函數
}
...
const EditTargetForm: React.FC<EditTargetFormProps> = ({ 
  ...
  useEffect(() => {
    if (target && open) {
      setFormData({
        ...
        startCultureDate: target.startCultureDate || '',
        generation: target.generation || undefined,
        boxCount: target.boxCount || undefined,
        expectedCompletionDate: target.expectedCompletionDate || '',
      });
      setErrors({});
   ...
   * @param value 欄位值
   */
  const handleFieldChange = (field: keyof UpdateTargetRequest, value: string | number | undefined) => {
    setFormData(prev => ({
      ...prev,
      [field]: value,
    }));
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name || formData.name.trim().length === 0) {
      ...
      newErrors.name = '目標名稱至少需要 2 個字元';
    } else if (formData.name.length > 100) {
      newErrors.name = '目標名稱不能超過 100 個字元';
    }

    // 驗證預計完成時間
    if (!formData.expectedCompletionDate || formData.expectedCompletionDate.trim().length === 0) {
    ...
    // 驗證代數
    if (formData.generation !== undefined && formData.generation < 1) {
      newErrors.generation = '代數必須大於 0';
    }

    // 驗證盒數
    if (formData.boxCount !== undefined && formData.boxCount < 1) {
      newErrors.boxCount = '盒數必須大於 0';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 呼叫 API 更新預生產目標
      const response = await TargetService.updateTarget(target.id, formData);

      if (response.success && response.data) {
        // 成功更新，觸發成功回調
        onSuccess(target.id, response.data);
        ...
        // 顯示錯誤訊息
        setErrorMessage(response.message || '更新預生產目標失敗');
      }
    } catch (error: any) {
      console.error('更新預生產目標錯誤:', error);
      setErrorMessage('更新預生產目標失敗，請稍後再試');
    } finally {
      setLoading(false);
    }
  };
